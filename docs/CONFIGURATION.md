...
The same offline SDK-profile and signing-key validation is enforced automatically before `check`, `status --refresh`, and `run --live`, before the production adapter can be constructed. Ordinary `status` remains configuration-free and local; dry-run never loads the OCI profile or constructs an adapter.
